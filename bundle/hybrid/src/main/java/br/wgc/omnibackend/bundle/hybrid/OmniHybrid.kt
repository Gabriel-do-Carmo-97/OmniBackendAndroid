...
object OmniHybrid {
     ...
     */
    fun createAuth(primary: AuthRepository, secondary: AuthRepository): AuthRepository = HybridAuthRepository(primary, secondary)
     ...
     */
    fun createDatabase(primary: FirestoreRepository, secondary: FirestoreRepository): FirestoreRepository =
        HybridDatabaseRepository(primary, secondary)
     ...
     */
    fun createStorage(primary: StorageRepository, secondary: StorageRepository): StorageRepository =
        HybridStorageRepository(primary, secondary)
}

/**
 * Implementação híbrida de [FirestoreRepository] com failover ativo-passivo entre dois provedores NoSQL.
 */
internal class HybridDatabaseRepository(private val primary: FirestoreRepository, private val secondary: FirestoreRepository) :
    FirestoreRepository {
    ...
    override fun <T : Any> listenToDocument(collection: String, documentId: String, clazz: Class<T>): Flow<DataResult<T?>> =
        primary.listenToDocument(collection, documentId, clazz)
 ...
 */
internal class HybridStorageRepository(private val primary: StorageRepository, private val secondary: StorageRepository) :
    StorageRepository {

    override fun uploadFile(path: String, fileData: ByteArray): Flow<DataResult<Uri>> = primary.uploadFile(path, fileData)

    override fun uploadFile(path: String, fileUri: Uri): Flow<DataResult<Uri>> = primary.uploadFile(path, fileUri)

    override fun uploadFile(path: String, inputStream: InputStream): Flow<DataResult<Uri>> = primary.uploadFile(path, inputStream)
