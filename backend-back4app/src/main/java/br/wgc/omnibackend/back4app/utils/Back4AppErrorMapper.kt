...
internal object Back4AppErrorMapper {
     ...
     */
    fun mapException(e: ParseException, context: String = "auth"): AppError {
        return when (e.code) {
            ParseException.OBJECT_NOT_FOUND -> {
                if (context == "auth") AppError.Auth.UserNotFound
                else if (context == "storage") AppError.Storage.ObjectNotFound
                else AppError.Firestore.DocumentNotFound
            }
            ...
            ParseException.OPERATION_FORBIDDEN -> {
                if (context == "firestore") AppError.Firestore.PermissionDenied
                else if (context == "storage") AppError.Storage.PermissionDenied
